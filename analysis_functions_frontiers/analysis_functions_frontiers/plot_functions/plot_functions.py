...
def plot_source_space(name, subject, subjects_dir, save_plots, figures_path):
    ...
    if save_plots:
        save_path = join(figures_path, subject, 'source_space', name + '.png')
        mayavi.mlab.savefig(save_path)
        print 'figure: ' + save_path + ' has been saved'
...
def plot_grand_averages_source_estimates_cluster_masked(name, 
    ...
    save_names = ['left_hemisphere.jpg', 'right_hemisphere.jpg']
    save_views = [[5.5, -78.0, 405.0], [5.5, 78.0, 405.0]]# ['medial', 'lateral']
    ...
    mayavi.mlab.close(None, True)
    clim = dict(kind='value', lims=[cutoff, cutoff, cutoff])
