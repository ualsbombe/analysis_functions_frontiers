...
script_path = '/home/lau/analyses/frontiers_group_analysis/scripts/python/'
chdir(script_path)
...
def populate_data_directory(home_path, data_path, figures_path, subjects_dir,
                            subjects_and_dates):
    
    ## create MEG and MRI paths   
    for subject_and_date in subjects_and_dates:

        full_path_MEG = join(home_path, data_path, subject_and_date)
        ## create MEG dirs
        try:
    ...
    grand_average_path = join(home_path, data_path, 'grand_averages/statistics')
    try:
    ...
    figure_subfolders = ['power_spectra_raw', 'evokeds', 'ica', 'stcs',
                         ...
                         'noise_covariance']
    for subject_and_date in subjects_and_dates:
        subject = subject_and_date[:11]
        for figure_subfolder in figure_subfolders:
            full_path_figures = join(home_path, figures_path, subject,
                                     figure_subfolder)
            ## create figure paths
            try:
    ...
    grand_averages_figures_path = join(home_path, figures_path,
                                      'grand_averages')
    figure_subfolders = ['sensor_space', 'source_space']
    for figure_subfolder in figure_subfolders:
        try:
...
def epoch_raw(name, save_dir, lowpass, event_id, tmin, tmax,
              baseline, reject, bad_channels, overwrite):
